...
public class PlayerInput : MonoBehaviour
{
	...
	public float maxTimeOtherHeal = 0.5f;
    ...
    void OnCollisionStay2D(Collision2D col){
	 ...
	 void OnCollisionEnter2D(Collision2D col){
		...
		if (Vector2.Angle(normal, new Vector2(0f, -1f)) < 10f)
			speed.y = 0;
		if(col.otherCollider.gameObject.tag == "PlayerAttack" && col.gameObject.tag == "Enemy"){ // rotina de ataque em inimigos normais
			if(timeOtherHeal <= 0){
				timeOtherHeal = maxTimeOtherHeal;
				health++;
			}
			col.gameObject.GetComponent<Enemy1Behavior>().TakeDamage(1);
		}
		if(col.otherCollider.gameObject.tag == "PlayerAttack" && col.gameObject.tag == "Boss" && timeOtherHeal <= 0){ // rotina de ataque no boss
			timeOtherHeal = maxTimeOtherHeal;
			health++;
    ...
    void FixedUpdate(){
		...
		//}
		 ...
		 timeOtherHeal -= Time.fixedDeltaTime;
	}
